// ============================================================================
// UNIFIED TYPES - SINGLE SOURCE OF TRUTH FOR ENTIRE APPLICATION
// ============================================================================
// This file consolidates ALL types from client, server, and WebSocket
...
export interface IShoppingList extends BaseDocument {
  ...
  description?: string;
  group: string;
  groupId: string; // Alias for group to maintain compatibility
  createdBy: string;
  status: "active" | "completed" | "archived";
  isActive: boolean; // Alias for status === 'active'
  isCompleted: boolean; // Alias for status === 'completed'
  items: string[];
  assignedTo?: string;
  dueDate?: Date;
...
export interface IItem extends BaseDocument {
  ...
  actualPrice?: number;
  price?: number; // Alias for estimatedPrice to maintain compatibility
  image?: string;
  barcode?: string;
  status: "pending" | "purchased" | "not_available" | "cancelled";
  isPurchased: boolean; // Alias for status === 'purchased'
  isPartiallyPurchased?: boolean; // True if 0 < purchasedQuantity < quantity
  purchasedQuantity?: number; // Quantity that has been purchased (0 to quantity)
  remainingQuantity?: number; // Quantity remaining to be purchased (quantity - purchasedQuantity)
  addedBy: string;
  purchasedBy?: string | null;
  purchasedAt?: Date | null;
  updatedBy?: string; // For tracking who last updated the item
  beingPurchasedBy?: string; // For real-time shopping status
  shoppingList: string;
  priority: "low" | "medium" | "high";
  notes?: string; // Alias for note to maintain compatibility
  note?: string; // Alias for notes to maintain compatibility
  alternatives?: string[];
  product?: string; // Product ID if linked to a product
  productId?: string; // Alias for product to maintain compatibility
  isManualEntry?: boolean; // True if manually entered, false if from product
}
...
export interface IWebSocketEvents {
  'list:updated': {
    listId: string;
    groupId?: string;
    action: string; // 'item_purchased' | 'item_unpurchased' | 'metadata_update' ...
    list: any;      // Full list data from server
  ...
  "shopping:started": {
    ...
    user: IUserSimple;
    startedAt: string;
    sessionId: string; // Added sessionId to match backend emit
  };
  "shopping:stopped": {
    listId: string;
    user: IUserSimple;
